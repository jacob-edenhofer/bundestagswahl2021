...
knitr::opts_chunk$set(echo = TRUE, fig.align = "center", echo = FALSE, fig.width = 6, fig.height = 4)
```
...
```{r parties-change-zweitstimmen, message=FALSE}
# data 
time <- read_delim("https://www.bundeswahlleiter.de/dam/jcr/0c8deca8-4030-419c-9fa8-5ab770cfa123/btw_rws_zwst-1953.csv",
                   skip = 11, col_names = T, locale = locale(decimal_mark = ",")) %>% 
  clean_names()
## variables 
time$age_coarse <- fct_collapse(factor(time$altersgruppe_etwa_von_bis_jahren),
...
```{r parties-differences-in-support, message=FALSE}
time %>% 
  filter(bundestagswahl %in% c(2017, 2021), grepl("Summe", geschlecht), 
         !grepl("Summe", age_coarse)) %>%
...
```{r parties-by-age-group, message=FALSE}
time %>%
...
```{r parties-gender-age-plot, eval=FALSE}
# data 
...
```{r parties-gender-age-plot-alt, message=FALSE}
time %>%
  filter(!grepl("Summe", geschlecht), !grepl("Summe", altersgruppe_etwa_von_bis_jahren),
         bundestagswahl == 2021) %>%
  mutate(age_coarse = fct_collapse(altersgruppe_etwa_von_bis_jahren, 
